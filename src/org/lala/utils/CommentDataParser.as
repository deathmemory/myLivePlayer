    ...
    public class CommentDataParser
    {
        ...
        public static function bili_parse(xml:XML,foo:Function):void
        {
            ...
            for each(var item:XML in list)
            {
				...
				obj.fulldate = fulldate(new Date(attrs[4] * 1000));
                obj.border = false;
                obj.id = length ++;
        ...
        **/
        public static function data_format(item:Object):Object
        {
            ...
            if(item.type == 'normal')
            {
				...
				if (item.mkjogo_u != null && item.mkjogo_s != null) {
					data.mkjogo_u = item.mkjogo_u;
					data.mkjogo_s = item.mkjogo_s;
				} else if (item.userid != null && item.session != null) {
