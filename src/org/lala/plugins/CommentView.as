...
//            clip.mask = clipMask;
        ...
        {
            navigateToURL(new URLRequest('http://www.nikksy.com/'), '_blank');
        }
        /** 点击右键后,开始生成菜单 **/
        ...
        private function setupUI():void
        {
            ...
            //_loopButtonIcon = new LoopButtonIcon() as Bitmap;	// 已经暂时换成了是否显示弹幕的按键 
            //cb.addButton(_loopButtonIcon,'LoopBt',loopButtonHandler);
            //_loopButtonIcon.alpha = player.config.repeat != 'single' ? 0.5 : 1;
			//_loopButtonIcon.alpha = CommentConfig.getInstance().visible ? 1 : .5;
            //使用ZIP皮肤时必须在装好按钮后设置
...
//            cb.addButton(_visibleButtonIcon,'VisibleBt',visibleButtonHandler);
        ...
        **/
        private function loopButtonHandler(event:Event):void
        {
			...
			var _clip:Sprite = CommentView.getInstance().clip;
